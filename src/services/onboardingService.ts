...
export async function initializeFirstConversation(userId: string) {
  ...
  // 1. Create the first "Sanctuary"
  const { data: conversation, error: convoError } = await supabaseAdmin
    .from("conversations")
    .insert({
      user_id: userId,
      title: "The First Mirror",
      system_prompt: process.env.SYSTEM_PROMPT // Use your unified one-liner
    })
    .select()
    .single();
  ...
  // 2. Insert the "Rules of the Room" as the first Assistant message
